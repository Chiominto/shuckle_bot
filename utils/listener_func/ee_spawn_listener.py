...
async def check_cc_bump_reminder(bot: commands.Bot, message: discord.Message):
    ...
    if embed_title == "Votes left until EE Spawn":
        ...
        if votes_left_int <= 100 and boss_key not in near_spawn_alert_cache:
            ...
            channel_id = CELESTIAL_TEXT_CHANNELS.bumps
            ee_ping_role_id = CELESTIAL_ROLES.ee_ping

            channel = clan_guild.get_channel(channel_id) or Object(id=channel_id)
...
async def send_cc_bump_reminder(
    ...
    cc_guild = bot.get_guild(CC_SERVER_ID)
    ...
    if context == "votes left" and votes_left:
        ...
        now = time.time()
        last_time = cc_shared_cooldowns.get(CELESTIAL_SERVER_ID, 0)
        ...
        cc_shared_cooldowns[CELESTIAL_SERVER_ID] = now
...
async def auto_wb_ping(
    ...
    variant_emoji = Emojis.shinygigantamax if variant == "shiny" else Emojis.gigantamax

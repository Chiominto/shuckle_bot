...
from constants.aesthetics import Emojis, Thumbnails
...
class ToggleIncensePingButton(discord.ui.View):
    ...
    async def toggle_incense_ping(
        ...
        incense_ping_role = interaction.guild.get_role(CELESTIAL_ROLES.incense_ping)
...
> - +10% increased {Emojis.Shiny} shiny rate (for `;pokemon` & `;fish`)
...
async def incense_use_handler(
    ...
    guild = message.guild
    bump_channel = guild.get_channel(CELESTIAL_TEXT_CHANNELS.bumps)
    incense_ping_role = guild.get_role(CELESTIAL_ROLES.incense_ping)
    if not incense_ping_role:
        pretty_log(
            "warning",
            f"Incense Ping role (id={CELESTIAL_ROLES.incense_ping}) not found in guild {guild.name} ({guild.id})",
        )
        return
    content = (
...
async def incense_command_handler(
    ...
    bump_channel = message.guild.get_channel(CELESTIAL_TEXT_CHANNELS.bumps)
    incense_ping_role = message.guild.get_role(CELESTIAL_ROLES.incense_ping)
    if not incense_ping_role:
        pretty_log(
            "warning",
            f"Incense Ping role (id={CELESTIAL_ROLES.incense_ping}) not found in guild {message.guild.name} ({message.guild.id})",
        )
        return
    if new_incense_msg:

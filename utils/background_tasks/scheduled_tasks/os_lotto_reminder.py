...
from constants.pokemon_gifs import GOLDEN_POKEMON_URL
from utils.functions.cleanup_first_match import cleanup_first_match
from utils.logs.pretty_log import pretty_log

CHANNEL_ID = CELESTIAL_TEXT_CHANNELS.bumps
OS_LOTTERY_ROLE_ID = CELESTIAL_ROLES.os_lottery
GUILD_ID = CELESTIAL_SERVER_ID
# Timezones
...
async def send_lotto_reminder(bot: discord.Client):
    ...
    embed = discord.Embed(
        description=(
            f"### 🎯 **OS LOTTO REMINDER**\n"
            f"- **1st Prize:** {Emojis.Golden} **{prize_info['name']}**\n"
    ...
    embed.set_footer(
        text="⏰ Don’t miss your chance – buy tickets before it closes!",
